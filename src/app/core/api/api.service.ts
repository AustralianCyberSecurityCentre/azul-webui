...
export class ApiService {
  ...
  private addReceivedSecurity(
    meta: components["schemas"]["Meta"] | null | undefined,
  ) {
    if (meta == null || meta == undefined) {
      console.warn("Api failed to provide metadata in response.");
      return;
    }
    const tmp = this.receivedSecurities$.getValue();
    if (meta.security && !tmp.has(meta.security)) {
      // only recompute if set has changed
      tmp.add(meta.security);
  ...
  ): Observable<components["schemas"]["SimilarFeatureMatch"] | undefined> {
